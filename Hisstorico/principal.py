import pygame
# Inicialización de Pygame
pygame.init()
# Inicialización de la superficie de dibujo
ventana = pygame.display.set_mode((640,480))
pygame.display.set_caption("Hispania")
#Creo el objeto caballero
caballero=pygame.image.load("img/caballero.png")
# Obtengo el rectángulo del objeto anterior
caballeroRect = caballero.get_rect()
# Posición inicial del caballero
speed = [4,4]
caballeroRect.move_ip(0,0)
# Bucle principal del juego
jugando = True
while jugando:
    # Comprobamos los eventos
    # Comprobamos si se ha pulsado el botón de cierre de la ventana
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            jugando = False
    
    caballeroRect = caballeroRect.move(speed)

    # Se pinta la ventana con un color
    # Esto borra los posibles elementos que teníamos anteriormente
    ventana.fill((255, 255, 255))
    # Todos los elementos del juego se vuelven a dibujar
    pygame.display.flip()
    # Controlamos la frecuencia de refresco (FPS)
    pygame.time.Clock().tick(60)
pygame.quit()